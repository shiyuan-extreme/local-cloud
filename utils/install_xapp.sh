...
rancher_install(){
	...
	rancher app install \
        ...
        --set xconf.acctServer=${GDC_SERVER} \
        --set xconf.loginUrl=${LOGIN_URL} \
        --set xconf.domainZone=${RDC_CUSTOMER_DOMAIN} \
        --set xconf.hmVersion=${XIQ_VERSION} \
        --set xconf.rdcName=${CLUSTER_NAME} \
		--namespace ${NAMESPACE}  \
		xapp ${APP_NAME}
}
